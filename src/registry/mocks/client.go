...
// PullImageInfo provides a mock function with given fields: keychain, transport, imageName
func (_m *MockImageGetter) PullImageInfo(keychain authn.Keychain, transport *http.Transport, imageName string) (*v1.Image, error) {
	ret := _m.Called(keychain, transport, imageName)

	var r0 *v1.Image
	var r1 error
	if rf, ok := ret.Get(0).(func(authn.Keychain, *http.Transport, string) (*v1.Image, error)); ok {
		return rf(keychain, transport, imageName)
	}
	if rf, ok := ret.Get(0).(func(authn.Keychain, *http.Transport, string) *v1.Image); ok {
		r0 = rf(keychain, transport, imageName)
	} else {
		if ret.Get(0) != nil {
	...
	if rf, ok := ret.Get(1).(func(authn.Keychain, *http.Transport, string) error); ok {
		r1 = rf(keychain, transport, imageName)
	} else {
		r1 = ret.Error(1)
	}
