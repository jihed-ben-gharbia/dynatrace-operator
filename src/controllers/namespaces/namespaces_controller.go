...
func namespaceCreatePredicate(controller *Controller) predicate.Predicate {
	...
	return func(createEvent event.CreateEvent) bool {
		...
		if labels != nil {
			if dynakubeName, ok := labels[dtwebhook.InjectionInstanceLabel]; ok {
				err := controller.reconcileDynakubeSecretsByName(context.TODO(), dynakubeName, controller.operatorNamespace)
				if err != nil {
...
func updateFunc(controller *Controller) func(updateEvent event.UpdateEvent) bool { // nolint:revive
	return func(updateEvent event.UpdateEvent) bool {
		...
		if newInstanceLabel != "" {
			err := controller.reconcileDynakubeSecretsByName(context.TODO(), newInstanceLabel, controller.operatorNamespace)
			if err != nil {
...
func (controller *Controller) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&source.Kind{Type: &corev1.Namespace{}}, &handler.EnqueueRequestForObject{}, builder.WithPredicates(namespaceCreatePredicate(controller))).
		Owns(&corev1.Secret{}).
		Complete(controller)
}
...
func (controller *Controller) reconcileDynakube(ctx context.Context, dynakubeName, dynakubeNamespace string) (reconcile.Result, error) {
	...
	if oldNamespaceSecretsPhase != dynakube.Status.NamespaceSecretsPhase {
		log.Info("status changed, updating DynaKube Status.NamespaceSecretsPhase")
...
func (controller *Controller) reconcileDynakubeSecretsByName(ctx context.Context, dynakubeName, dynakubeNamespace string) error {
	log.Info("reconciling DynaKube (secrets)", "namespace", dynakubeNamespace, "name", dynakubeName)
	...
	_, err = controller.reconcileDynakubeSecrets(ctx, dynakube)
